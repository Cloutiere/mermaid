...
from app.services.subgraphs import (
    create_subgraph,
    get_subgraph_by_id,
    update_subgraph,
    delete_subgraph,
...
def update_subgraph_route(subgraph_id: int):
...
def assign_nodes(subgraph_id: int):
    ...
    updated_subgraph = assign_nodes_to_subgraph(subgraph_id, data.node_ids)
    response_data = SubgraphRead.model_validate(updated_subgraph).model_dump()
    return jsonify(response_data), 200

@subgraphs_bp.route('/<int:subgraph_id>/unassign_nodes', methods=['PATCH'])
def unassign_nodes_route(subgraph_id: int):
    ...
    updated_subgraph = unassign_nodes_from_subgraph(subgraph_id, data.node_ids)
    response_data = SubgraphRead.model_validate(updated_subgraph).model_dump()
    return jsonify(response_data), 200